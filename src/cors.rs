use tide::http::headers::{HeaderValue, HeaderValues};
...
impl CorsMiddleware {
    ...
    fn build_preflight_response(&self, origins: &HeaderValues) -> tide::http::Response {
...
impl<State: Clone + Send + Sync + 'static> Middleware<State> for CorsMiddleware {
    ...
    async fn handle(&self, req: Request<State>, next: Next<'_, State>) -> Result {
        ...
        if origins.is_none() {
            debug!("No Origin header, allowing the request");
        ...
        if !self.is_valid_origin(&origin) {
            error!("Origin is invalid, returning 403");
            return Ok(tide::http::Response::new(StatusCode::Forbidden).into());
        }

        // Return results immediately upon preflight request
        if req.method() == Method::Options {
            debug!("Preflight request, returning immediately");
...
mod test {
    ...
    fn request() -> tide::http::Request {
        let mut req = tide::http::Request::new(tide::http::Method::Get, endpoint_url());
        req.insert_header(tide::http::headers::ORIGIN, ALLOW_ORIGIN);
        req
    }
    ...
    async fn preflight_request() {
        ...
        let mut req = tide::http::Request::new(tide::http::Method::Options, endpoint_url());
        req.insert_header(tide::http::headers::ORIGIN, ALLOW_ORIGIN);
    ...
    async fn default_cors_middleware() {
        ...
        let res: tide::http::Response = app.respond(request()).await.unwrap();
    ...
    async fn set_allow_origin_list() {
        ...
        for origin in origins {
            let mut req = tide::http::Request::new(tide::http::Method::Get, endpoint_url());
            req.insert_header(tide::http::headers::ORIGIN, origin);
    ...
    async fn not_set_origin_header() {
        ...
        let req = tide::http::Request::new(tide::http::Method::Get, endpoint_url());
        let res: tide::http::Response = app.respond(req).await.unwrap();
    ...
    async fn unauthorized_origin() {
        ...
        let mut req = tide::http::Request::new(tide::http::Method::Get, endpoint_url());
        req.insert_header(tide::http::headers::ORIGIN, "unauthorize-origin.net");
        let res: tide::http::Response = app.respond(req).await.unwrap();
    ...
    async fn set_cors_headers_to_error_responses() {
        let mut app = tide::Server::new();
        app.at(ENDPOINT).get(|_| async {
            Err::<&str, _>(tide::Error::from_str(StatusCode::BadRequest, "bad request"))
        });
